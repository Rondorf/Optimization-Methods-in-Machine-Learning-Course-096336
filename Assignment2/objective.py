...
class OracleLS:
    ...
    def get_stochastic_grad(self, x, batch_size=1):
        random_subset_idx = random.sample(range(self.A.shape[0]), batch_size)
        sub_A = self.A[random_subset_idx]
        sub_b = self.b[random_subset_idx]

        return (1 / batch_size) * sub_A.T.dot(sub_A.dot(x) - sub_b)

    def get_stochastic_grad_by_index(self, x, idx):
        ' get stochastic gradient g_i for chosen input i'
        sub_A = self.A[idx]
        sub_b = self.b[idx]
        return sub_A.T.dot(sub_A.dot(x) - sub_b)
